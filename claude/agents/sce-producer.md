...
- **Identifier names are semantic, not knowledge-graph coordinates.** NEVER name a function, method, class, type, module, or variable after the location of its source-of-truth document — patterns like `classifyPerArch038Section42`, `emitForR012Section1AC03`, `validateDC07a`, or `processForArch038Sec42` are forbidden. Spec linkage belongs in `@implements`/`@see`/`@depends-on`/`@validates` annotations only. The encoded text in an identifier cannot participate in `scepter claims trace`, silently rots when the cited section renumbers, and pollutes call sites. When deriving from authoritative-delegation claims like "follows X verbatim," resist the instinct to embed the source coordinate in the identifier — name the function for what it does (`classifyByDispatchTable`), then let the `@implements` / `@see` annotation cite the source. Test descriptions of the form `test('S002.§1.AC.01: ...', ...)` are the documented exception. See `implementing.md` § Identifier Names Are Semantic for the full rule.
