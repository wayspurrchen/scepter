---
name: scepter
description: Core SCEpter concepts including the knowledge graph system, note types, and configuration-driven architecture. Foundational understanding required for all SCEpter work. This is the main entry point for SCEpter knowledge — including adopting/retrofitting SCEpter onto an existing project (analyzing it, proposing note types, ingesting docs) and bootstrapping a project's cold-start surface (the FIRST ACTION RULE and allowlist). Invoke this skill for "retrofit", "bootstrap", or "set up scepter here".
allowed-tools: Read, Write, Edit, MultiEdit, Glob, Grep, Bash, Task, NotebookEdit, TodoWrite, WebFetch
---

# SCEpter

## NON-NEGOTIABLE RULES

These rules prevent the most common agent failures. Violating any of them will break the knowledge graph or produce wrong output.

1. **CONFIG FIRST** — Run `scepter config` before ANY action if you have not confirmed the note types and paths. Never assume note types, shortcodes, folder structures, or storage locations exist. In particular, never assume notes live under `_scepter/` — discovery paths are configurable and notes can live anywhere in the project.
2. **CLI ONLY** — Never use Read/Glob/Grep on note files for discovery. Always use `scepter ctx` commands (`list`, `show`, `search`, `gather`). The CLI knows where notes live; you don't. Exception: editing a file AFTER finding it with CLI (read the file first after using `scepter ctx create` to avoid update errors).
3. **NEVER GUESS IDs** — IDs are auto-generated by `scepter create`. Never predict the next ID, never manually create note files on disk, never pre-write references like `{D005}` before the note exists. Create first, read ID from output, then reference.
4. **GATHER BEFORE WORKING** — `scepter ctx gather [TASK_ID]` is MANDATORY before starting any task. Never work without full context.
5. **CREATE IMMEDIATELY** — Decision made → create decision note NOW. Question arises → create question note NOW. Don't defer.
6. **CONNECT EVERYTHING** — Every note must reference related notes. Isolated notes are useless. Build the graph actively.
7. **DATE EVERYTHING** — Run `date "+%Y-%m-%d"` before EVERY edit. Use the exact output. Never use "now", "today", or made-up dates.
8. **REFERENCE IN CODE** — All code implementing SCEpter notes MUST include `@implements`, `@depends-on`, `@see`, or `@addresses` comments with note IDs.
9. **USE CLAIM REFERENCES** — Use fully qualified claim paths: `{R012.§1.AC.15}` not bare `AC.15`. The dot between prefix and number is mandatory (`AC.01` not `AC01`). Hyphens are forbidden (`AC.01` not `AC-01`). **Read `claims.md` from this skill directory for the full claim system.**
10. **LINKER AFTER WORK** — After any substantial work cycle that touches SCEpter notes or `@implements`-annotated code (producer dispatch, implementation pass, arc close, multi-step edit), the orchestrator MUST dispatch `sce-linker` for graph hygiene. This applies to ALL dispatch paths — team protocol, direct sce-producer, implement-review, manual implementation, all of them. Dispatch with `run_in_background: true` so the linker doesn't block the next user-facing decision. The linker handles missing `@implements` annotations, bidirectional ref hygiene, supersession lifecycle tags, and cross-references between produced notes and adjacent graph context.
11. **REVIEW FOLLOWUP — ROUTE BY JUDGMENT TYPE** — When `sce-reviewer` (or any review pass) returns findings, classify each finding before acting. **Mechanical findings** (typos, citation updates, missing bidirectional refs, stub-`@implements`-to-`@see` conversions per claims.md, format alignment, applying a clear rule consistently, reverting silent deviations from cited sources, fleshing out skeletal/placeholder authoring) → dispatch `sce-producer` with the specific finding(s) as the unit of work; apply, then summarize what was applied in your next user-facing message. Do NOT surface mechanical fixes for per-item user ack. **Human-judgment findings** (scope changes, classification framings, naming alternatives, design choices between equally-valid options, "Q-note vs DEF vs other note type" framings — anything where two reasonable readers could pick different answers) → surface to user with the finding and the options. Do NOT auto-apply. **Failure-to-follow-source is NOT human judgment.** When an artifact deviates from a cited source (wrong attribution, invented content, silent rename, divergence from prior conventions) or gestures at content that doesn't exist (`(...all other rows...)` placeholders, dangling self-references, skeletal sections), the default is MECHANICAL: revert the deviation or flesh out per the source. Escalate to HUMAN_JUDGMENT only when the reviewer can argue the deviation is genuinely better than the source OR has prior user authorization on record — and the justification must be explicit. The failure mode this prevents: (a) over-pausing — surfacing every finding for ack, which wastes user attention on routine fixes; (b) over-applying — silently routing judgment-laden findings as if mechanical, which strips the user's substantive review; (c) laundering authoring failures as design questions — surfacing "should we use the invented namespace or the cited one?" as if both were equally-valid options when one is a hallucination. Default lean for genuine ambiguity: surface. Default lean for source-deviation or skeletal authoring: revert/flesh-out as MECHANICAL. Reviewers are asked to tag findings `MECHANICAL` or `HUMAN_JUDGMENT` (see `reviewing.md` § Marking Findings); untagged findings default to HUMAN_JUDGMENT, but reviewers should not tag source-deviations or skeletal authoring as HUMAN_JUDGMENT without an explicit "genuinely better" or "user-authorized" argument.

## Companion Files

This skill directory contains companion files with deeper guidance. **Read them with the Read tool when needed.**

Path: `~/.claude/skills/scepter/<filename>.md`

```
WHAT OPERATION ARE YOU PERFORMING?
│
├─ PRODUCING an artifact (code, design, requirements, specs, tests)?
│   └─ **DISPATCH the sce-producer agent.** Do not do this work inline.
│      Read claude/agents/sce-producer.md and launch with Agent tool.
│      If the team protocol is in play (team.md): production is a producer+reviewer
│      PAIR on a team — TeamCreate + spawn BOTH concurrently. A lone producer is a
│      protocol violation. See team.md § Orchestrator Bootstrap.
│
├─ REVIEWING or VALIDATING work?
│   └─ **DISPATCH the sce-reviewer agent.** Do not review inline.
│      Read claude/agents/sce-reviewer.md and launch with Agent tool.
│
├─ ADOPTING SCEpter on a project — analyze it, propose note types, ingest docs?
│   ("retrofit", "set up scepter here", "establish a knowledge graph")
│   └─ Read retrofit.md  (→ retrofit-analysis.md for Phase 1, retrofit-proposal.md for Phase 2)
│
├─ INSTALLING the cold-start surface into a project — FIRST ACTION RULE in CLAUDE.md,
│   allowlist, plugin/skill wiring ("bootstrap" a project to auto-load this skill)?
│   └─ Read bootstrap.md
│
├─ Multi-step feature work?  → Read process.md
├─ Working in an agent team?  → Read team.md
├─ See claim IDs?            → Read claims.md
│
├─ AUTHORING claims (requirements, specs, DDs)?
│   ├─ Read claims.md (syntax, metadata, derives=)
│   ├─ Need epistemic vocabulary (binding, inherence, projection, etc.)? → Read epistemic-primer.md
│   └─ Producing a formal document? → Read artifacts/{type}.md
│      (requirements | specification | architecture | detailed-design | test-plan | implementation)
│
├─ DERIVING (concretizing claims to a new projection)?
│   ├─ To code → Read implementing.md + claims.md
│   ├─ To a DD/spec/other doc → Read claims.md
│   └─ High-binding AC (maps to 4+ files)? Decompose with derives=TARGET
│
├─ REVIEWING a claim stack?
│   ├─ Completeness (is the AC set sufficient?) → Read reviewing.md
│   ├─ Conformance (does X match Y?)           → Read conformance.md
│   ├─ Impact (what did my change affect?)      → Read implementing.md `## Impact Analysis`
│   └─ Coherence (do projections agree?)        → Read reviewing.md
│
├─ IMPLEMENTING from a plan?
│   └─ Read implementing.md (or dispatch sce-producer for the work)
│
├─ CONNECTING (linking claims across artifacts)?
│   ├─ In code → Read implementing.md `## Source Code References`
│   └─ In notes → This file: `## Knowledge Graph`
│
├─ VERIFYING / RETIRING / PROPAGATING claims?
│   └─ Read claims.md `### Lifecycle Tags` + `### CLI Tools`
│
└─ Everything else?          → This file has what you need
```

**Agent dispatch:** The agents at `claude/agents/sce-producer.md` and `claude/agents/sce-reviewer.md` are specialized for SCEpter work. When dispatching, read the agent file and pass its content as context to the Agent tool. Production work (writing code, authoring documents, implementing designs) should always be delegated to sce-producer rather than done inline — this keeps traceability annotations consistent and frees the orchestrating agent to manage the process.

## What is SCEpter?

SCEpter is a CLI-first knowledge management system that solves context loss and decision amnesia in AI-assisted development. It captures atomic units of project knowledge as markdown notes with unique IDs, creating a persistent knowledge graph through bidirectional references.

**Core problems solved:**
- Every new AI conversation starts with zero context
- Past decisions and rationale get buried
- AI recreates existing solutions
- Changes ripple without traceability

**How it works:** Notes → References → Knowledge Graph → CLI queries → Context for AI agents

**Key differentiator — Claim-level traceability:** SCEpter doesn't just link documents; it tracks individual claims (acceptance criteria, constraints, specifications) across projections. A requirement's `§2.AC.03` can be traced through the specification, into source code `@implements` annotations, and into test `@validates` markers. This is one of SCEpter's strongest value adds — agents are encouraged to use claim-level references (`{R004.§1.AC.03}`) over note-level references (`{R004}`) whenever the specific claim is known. See `claims.md` in this skill directory for the agent-facing summary.

## Epistemic Stance

SCEpter's purpose is to make claim-level trust state explicit across time, actors, and projections. Agents operating in SCEpter must match this stance — paranoid by default, deferential to recorded evidence, explicit about the distinction between what the agent produced and what the user authorized.

### Paranoia when consuming

When reading notes, handoffs, checkpoints, gathered context, or another agent's output, treat every assertion about prior state as tentative until verified. The presence of a claim in a polished document is not evidence for its truth — it is evidence that someone wrote those words. Apply the same posture to your own prior output when you re-read it in a new turn.

Verification sources that count:
- A verbatim user utterance in the current session
- A recorded event on the claim (`scepter claims verify`, trace output showing coverage)
- A grep in the code root that confirms a primitive exists at the cited file:line
- Derivation from another claim that itself meets one of the above

Prose attribution in a document, summary, or handoff is NOT verification. It is a claim expression that needs verifying.

### Don't speak for the user

When describing prior state, attribute to the artifact, not the user: "the DD states X," "the handoff claims X," "the log records X," "the claim reads X." Reserve "you said" / "you wanted" / "we agreed" for verbatim utterances in the current session. A paraphrase of a paraphrase is not a user utterance even when repeated across many documents.

When summarizing session context or carrying scope forward, cite the source of each user-attribution inline: verbatim quote, event record, or explicitly user-authored note. Unsourced user-attributions must be flagged as synthesized, not carried forward as user-sourced.

### Unmarked claims are tentative by default

The natural reading of unmarked prose is "the author stands behind it." For SCEpter claims, invert this default. A claim with no explicit origin marker and no recorded endorsement is agent-drafted and unreviewed. Trust is earned by explicit markers and recorded events, never assumed from polish, from repetition, or from a claim's appearance in a polished document.

### The trace matrix tracks expressions, not truth

This is axiomatic. `scepter claims trace` shows which projections have claim expressions; it does not show whether the claims are true, correctly realized, or user-endorsed. Consequences:
- A green trace matrix with full `@implements` coverage does not prove the system is correct.
- `@implements` presence is evidence that someone wrote the annotation, not that the user authorized the claim.
- Coverage metrics are necessary but not sufficient for trust. Pair with event-based endorsement and mechanical verification.

See `claims.md` § Smuggling for the attribution-side failure mode that these disciplines exist to prevent, and the general principle (a label is a factual claim, not a convenience marker) that smuggling, stub `@implements`, and reality gaps are all specific cases of.

## Core Architecture

### Notes as Atomic Knowledge Units

Notes follow Zettelkasten principles — as small as possible to capture a core idea, as large as necessary to capture it fully. Each note has a unique ID and type, and lives in a type-specific folder.

**Note types** are configured per project in `scepter.config.json`:
```json
{
  "noteTypes": {
    "Decision": { "folder": "decisions", "shortcode": "D", "description": "..." },
    "Requirement": { "folder": "requirements", "shortcode": "R", "description": "..." }
  }
}
```

**Storage is configurable, not fixed.** Notes are discovered by their ID prefix across the `discoveryPaths` listed in `scepter.config.json`. In a default project they live under `_scepter/notes/[type_folder]/`, but a project may scatter notes across `docs/`, `specs/`, or the entire repo (`["."]`). **Always use `scepter config` and `scepter ctx list` to discover where notes actually are — never hard-code a path.**

The `_scepter/` folder itself still holds project-level state (config, templates, verification data), but it is not necessarily where notes live.

**Tasks** use shortcode `T` and sequential IDs (T001, T002...). The `Task` type (and its storage folder) should be defined in the project's `scepter.config.json` like any other type — check with `scepter config` first. Tasks bridge knowledge (decisions, requirements) and work (implementation).

### Configuration-Driven Everything

Every SCEpter project is unique. Never assume what types exist — always run `scepter config` first. Configuration defines note types, folder structure, source code scanning, and status validation.

**Status configuration** supports:
- **Status sets**: reusable groups of statuses (e.g., "workflow": ["pending", "in-progress", "completed"])
- **Per-type allowed statuses** with `suggest` (warn) or `enforce` (block) modes
- **Default values** applied when no status is provided

## Knowledge Graph

### Bidirectional References

When you write `{D001}` in a note, two things happen: your note gains an outgoing reference to D001, and D001 gains an incoming reference from your note. Both directions are queryable.

### Progressive Disclosure

Three levels of detail:
1. **List**: Quick overview (ID, title, status, reference counts)
2. **Show**: Full note content with reference excerpts
3. **Gather**: Origin note + all related notes following reference chains

Control depth with `--depth N`, limit with `--max-notes`.

## Source Code Integration

SCEpter tracks bidirectional references between notes and source code. In code, reference notes via comments:

```typescript
/**
 * @implements {R001} User authentication requirement
 * @depends-on {D005} JWT decision
 * @see {T025} Implementation task
 */
class AuthService { ... }
```

**Reference types:** `@implements`, `@depends-on`, `@addresses`, `@validates`, `@see`, plain `{ID}`.

**Claim-level references** use dot notation: `{R012.§1.AC.15}`. Compact syntax for multiples: `{R012.§1.AC.11,.AC.29,.AC.30}`. Range syntax is supported: `{R012.§1.AC.01-30}`. The dot between prefix and number is mandatory — `AC01` and `AC-01` are both invalid. References that begin with a kebab-case alias (`vendor-lib/R005.§1.AC.01`) are cross-project citations that resolve against a peer SCEpter project; see `claims.md` "Cross-Project References" for the rules. See `claims.md` in this skill directory for the full syntax, rules, and examples.

**Cross-project annotations:** `@implements` and `@see` MAY use alias-prefixed targets to cite a peer project's claim — but `derives=` and `superseded=` MUST NOT (linter errors `cross-project-derives` and `cross-project-superseded`). The derivation graph is per-project; lifecycle authority is per-project.

**In tests:** `test('S002.§1.AC.01: eq matches identical values', () => { ... });`

**In documentation:** `This implements {R012.§1.AC.15} acceptance criteria.`

## Note Lifecycle Vocabulary

Four lifecycle moves retire or relocate a note. They are not interchangeable; pick the one whose intent matches the change.

| Move | Command | Inbound refs | Recovery surface |
|---|---|---|---|
| **Archive** | `scepter archive <ID>` | Unchanged (still resolve) | Note moves under archive folder; remains referenceable |
| **Soft-delete** | `scepter delete <ID>` (default) | Unchanged (still resolve) | Note moves under `_deleted/`; `restore`/`purge` recover or finalize |
| **Hard-delete** | `scepter delete --hard <ID>` | Rewritten to `_deleted_<ID>_at_<TS>` marker | None — file is gone; marker is provenance only |
| **Rename** | `scepter rename <OLD> <NEW>` | Rewritten to new ID silently | N/A — same note, new identifier |

Default to archive when uncertain. Hard-delete is for when leaving inbound references silently pointing at a phantom note would be worse than tombstoning them loudly. Rename is for relocating identity, not retiring meaning.

**Two distinct lifecycle surfaces exist; do not conflate them:**

- **Claim-level** (`:removed`, `:superseded=TARGET`): an author-applied metadata tag on a single claim line. The note still exists; one claim within it is retired.
- **Note-level** (`_deleted_<ID>_at_<TS>` deletion marker): a rewriter-applied substitution into every inbound reference when `scepter delete --hard` retires a whole note. The note's file is gone; the marker remains as the parser-invisible lifecycle state on every consumer.

`:removed` is something an author writes into a claim line. The deletion marker is something the rewriter writes into every other note that cited the deleted note. They live at different layers. Authors do not hand-write deletion markers — the rewriter is the sole producer.

For the detailed marker format, parser-invisibility property, consumer behavior, and the "tombstoned references are NOT broken references" discipline, see `claims.md` § Tombstoned References (Note-Level Lifecycle).

## The SCEpter Workflow Loop

1. **Capture** — Decision made → create note. Requirement found → create note. Question arises → create note.
2. **Connect** — Link decisions to requirements, tasks to specs, questions to answers.
3. **Use** — `scepter ctx gather` before starting work. Review all related context.
4. **Evolve** — Question answered → update with decision reference. Task done → mark status. Decision revised → new note referencing old.

## CLI Reference

The full SCEpter CLI command catalog lives in `~/.claude/skills/scepter/cli.md`. **MUST-load that file the first time your dispatch invokes a `scepter` command** — do not improvise commands or guess flag syntax. Producers, reviewers, and researchers all run CLI commands routinely (`scepter create`, `scepter ctx gather`, `scepter claims trace/gaps/lint/stale`, `scepter ctx show/search`). The reference is mandatory; load timing is lazy.

Quick orientation:
- Context subcommands work at the top level: `scepter list` = `scepter ctx list`
- ALWAYS run `scepter config` first to see project-specific note types, shortcodes, and status modes
- ALWAYS search before creating to avoid duplicates
- Note IDs are auto-generated by `scepter create` — never guess them; read the ID from CLI output

For full command syntax, flags, and examples, see `cli.md`.

## Status Management

The frontmatter-first rule, completion-authority hierarchy, artifact-authoring status discipline, and progress-note conventions live in `~/.claude/skills/scepter/status-management.md`. **MUST-load that file whenever you are about to flip a status or author a new artifact.**

Headlines:
- Frontmatter `status` is updated FIRST, then progress notes describing the change
- `completed` / `done` / `approved` are USER-ONLY transitions; agents stop at `ready_for_review`
- Producers leave new artifacts at the pending-equivalent default — never author with `status: accepted`

For the full rules, transition matrix, and examples, see `status-management.md`.

## Common Anti-Patterns

### 1. Reading Note Files Directly

❌ `Read: _scepter/notes/decisions/D001.md` / `Glob: _scepter/**/*.md` / `Glob: docs/**/*.md`
✅ `scepter ctx show D001` / `scepter ctx list` / `scepter ctx search "term"`

**Why:** CLI tracks references, resolves types, provides context. Direct access breaks all of this. It also fails when the project uses non-default `discoveryPaths` — notes may live in `docs/`, `specs/`, or anywhere configured. Only the CLI knows the configured locations. If you genuinely need the path of a note you've already identified, `scepter show` prints it.

### 2. Creating Without Searching First

❌ `scepter create Decision "Use PostgreSQL"` (didn't check existing)
✅ `scepter search "postgresql"` then `scepter list --types Decision --tags database` then create only if no matches.

### 3. Isolated Notes (No References)

❌ `# D001 - Use React` / `We'll use React.` (no connections)
✅ `Based on {R001} for SPA requirement. Answers {Q003}. Implementation in {T030}.`

### 4. Fake Dates

❌ "Updated just now" / "Completed today" / Making up dates
✅ `date "+%Y-%m-%d"` → use exact output: `2025-11-12`

### 5. Self-Completing Tasks

❌ `status: completed` (set by AI)
✅ `status: ready_for_review` (let user verify and complete)

### 6. Guessing Note IDs

❌ Creating note then writing `{D005}` assuming that's the ID
✅ `scepter create Decision "..."` → read ID from output (might be D003!) → then use `{D003}` in references

### 7. Preserving Dead Provenance When Editing

❌ `### Query-expressible methods (currently classified as "distinct-purpose" — revised 2026-03-28)`
✅ `### Query-expressible methods`

**Why:** When correcting, updating, or rewriting a document, do NOT preserve the old wrong state inline. A note saying "this was previously classified as X" has zero value when the reclassification happened in the same session or has no downstream consumers who relied on the old classification. It wastes tokens, confuses future readers, and creates noise that looks like it might be load-bearing history when it isn't.

**The test:** Does the old state have a provenance chain that someone might need to trace? Did other documents, code, or decisions depend on the old classification? If yes, the history matters — capture it in a status update, a decision note, or a checkpoint. If no (the correction is within the same working session, or no one ever acted on the wrong version), just write the correct thing. The git history preserves what changed; the document should reflect what IS.

**This applies to:** Section headers, claim text, architectural descriptions, any prose being corrected. It does NOT apply to lifecycle-tagged claims (`:removed`, `:superseded`) — those use the claim lifecycle system, not inline annotations.

## Note Creation Guidelines

**Triggers:** Decision made → Decision note (where the decision warrants tracking — see threshold below). Requirement discovered → Requirement note. Question arises → Question note. Work identified → Task.

**Timing:** IMMEDIATELY. Not "later."

**Quantity:** Create ONLY what was requested. Don't proliferate companion notes without permission.

**Content:** Capture what was STATED, not what you think should exist. Note questions to clarify rather than inventing specifications.

**After creation:** Edit the file to add content and `{ID}` references. Always date updates.

### When a decision warrants a Decision note

The default lean is to create one when the trigger fires, but the threshold is non-trivial. Decision notes record decisions that downstream readers might reasonably want to revisit, second-guess, or understand the rationale of. They are not journals of every choice.

**Create a decision note when:**
- The choice affects long-standing architecture, public API surface, operational practice, or a contract that other code/agents depend on.
- Multiple reasonable options exist and one was selected for stated reasons. The justification is what the note carries — without it, future agents have to reconstruct the rationale or re-litigate the choice.
- The decision creates downstream constraints (e.g., "we will always do X this way") that other work must respect.
- The decision overrides or supersedes an earlier decision, even implicitly.

**Skip the decision note when:**
- The original behavior was simply wrong (e.g., a typecheck error, a misimplementation, a bug). Picking the correct behavior is not a "decision" — it's a fix. The fix lives in the code and a progress note on the relevant task; a standalone D-note adds noise.
- One option is clearly correct and the other options are bugs / mistakes / non-options. There is no "we could go either way" tension to record.
- The choice is fully captured by the code change itself, with no rationale a future reader would need.
- The decision is a routine implementation detail invisible to consumers (variable names, internal helper structure, loop ordering).

**Heuristic test:** If you removed the decision note's content, would a future agent re-encountering this area face a choice with multiple reasonable answers and no recorded justification? If yes, write the note. If the only honest content of the proposed note is "we did the obviously correct thing instead of the wrong thing," skip the note and let the code + progress note carry it.

This threshold matters because decision-note proliferation degrades the graph: the signal (real architectural decisions worth knowing about) gets diluted by routine bug-fix entries, and the proper-noun authority of D-notes erodes. A graph with 100 D-notes where 60 are routine fixes is harder to navigate than one with 40 substantive decisions.
