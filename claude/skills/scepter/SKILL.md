...
1. **CONFIG FIRST** — Run `scepter config` before ANY action if you have not confirmed the note types. Never assume note types, shortcodes, or folder structures exist.
2. **CLI ONLY** — Never use Read/Glob/Grep on `_scepter/` files for discovery. Always use `scepter ctx` commands. Exception: editing a file AFTER finding it with CLI (read the file first after using `scepter ctx create` to avoid update errors).
3. **NEVER GUESS IDs** — IDs are auto-generated by `scepter create`. Never predict the next ID, never manually create files in `_scepter/`, never pre-write references like `{D005}` before the note exists. Create first, read ID from output, then reference.
...
### 1. Reading `_scepter/` Files Directly
