...
| Source ↔ Tests | Do tests verify the behavior the code claims — reaching the path with realistic fixtures and asserting on it so they fail on regression? A name-only or no-assertion test is not coverage (see conformance.md § Behavioral-Reachability Conformance Pass). |
...
**A checkable fact is NOT human judgment.** If a finding's routing — or the weight of a caveat you attach to it — depends on an unverified fact your tools can resolve (a code read, a grep, a config lookup), resolve it before tagging. Do not route to HUMAN_JUDGMENT, and do not attach a "verify that X" caveat, on the strength of an unread file. A resolvable unknown is a task to complete, not a gap to report; the conservative "when in doubt, surface" default applies only to unknowns that survive your own investigation, never as a license to skip a check the tools make cheap. Tell: calling a check "non-trivial" or "the hard part" without having attempted it is cost-inflation to justify not looking. This is the fact-shaped sibling of the source-deviation clause above — both are findings that *look* like human judgment but are not; routing either to the user wastes attention and creates false bilateral framing.
...
| Tombstoned reference flagged as broken | Reviewer treats `_deleted_<ID>_at_<TS>` as a stale or invalid reference; or conflates note-level tombstoning with claim-level `:removed` / `:superseded=` | Tombstoned refs are a recognized lifecycle state — `lint`, `trace`, `gaps` silently accept them. A deriving claim with a tombstoned target surfaces under `orphan-derives`, which is an authoring decision (re-derive, retire, replace), not a broken-ref fix. See `claims.md` § Tombstoned References (Note-Level Lifecycle) |
| Coverage theater / unreached behavior | A test's name or docstring claims a behavior, but its fixtures seed degenerate data that never reaches the path and no assertion checks it; or a code branch for a claimed case is unreachable under realistic input. Coverage reads green; the behavior never runs | Run conformance.md § Behavioral-Reachability Conformance Pass: for each claimed case construct the input that should reach it, and confirm both the code path AND a fail-on-regression assertion actually execute. Highest priority for bulk-mutation / migration / remediation code |
