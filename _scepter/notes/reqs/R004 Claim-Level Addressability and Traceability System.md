...
- {T012} — bug fix restoring §2.AC.03 conformance in the source-annotation projection: the `note-parser.ts` mention-scan regex dropped the claim path on section-less `§`-before-prefix citations (e.g. `{DD018.§DC.06}`), degrading them to note-level mentions and hiding claim-level `@implements` from `trace`/`gaps` coverage (added 2026-07-06)
