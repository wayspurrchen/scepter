...
The `allowedStatuses` / `StatusValidator` feature shipped before this requirement was authored, under code annotations that cited a phantom `{T011}` identifier ("Status Validation Service"). No `T011` note exists in this project — the real taxonomy is A/R/S/DD/TS/T and `T011` was never created. This requirement is the authored home for that work; the `{T011}` family of code annotations has been re-pointed to R019 claims. Implementation sites:
