...
The same badge primitive — count plus source-vs-note color encoding — also surfaces in two markdown-preview hover contexts: the claim-hover header (mirroring the inline badge for the cited claim) and the note-hover header (an aggregated count across the note's surface, see §1.AC.10). The editor hover surface does NOT carry these hover badges (see §1.AC.11) — its MarkdownString renderer strips inline-badge styles and the metadata is already surfaced textually.
...
§1.AC.09 The markdown preview's claim hover (rendered by `media/preview-script.js` `renderClaimHover`) MUST display the `●N` badge in its header next to the FQID, with the same source/note color encoding as the inline rendering. Count semantics MUST match §1.AC.02: total inbound = source-code refs + note-to-note refs to that specific claim. The badge MUST appear in both the original-claim mode (single-pane hover header) and the reference-to-claim mode (the body-panel header that names the cited FQID). When the claim has zero inbound references the badge MUST be omitted, matching §1.AC.07.

§1.AC.10 The markdown preview's note hover (rendered by `media/preview-script.js` `renderNoteHover`) MUST display an aggregated `●N` badge next to the note title. The count MUST equal the total of: (a) source-code references targeting the note id directly, (b) note-to-note references targeting the note id directly, (c) source-code references targeting any claim defined in the note, and (d) note-to-note references targeting any claim defined in the note. Color encoding MUST be green when at least one contributing reference is source-code (any of (a) or (c)), and red when every contributing reference is note-to-note. The badge expresses "how much of the world cites this note's surface in any way" — it is the note-level analogue of the per-claim badge. When the aggregated count is zero the badge MUST be omitted.

§1.AC.11 §1.AC.09 and §1.AC.10 apply to the markdown preview tooltip ONLY. The editor hover surface (`vscode/src/hover-provider.ts`) MUST NOT add inline `●N` badges to claim hovers or note hovers — VS Code's `MarkdownString` renderer strips much of the styling that would make the badge visually distinct, and the inbound-reference information is already surfaced textually in the editor hover's refs panel.
...
§3.AC.09 The note hover MUST render with the following layout, distinct from the claim hover's two-column structure:

- **Header line:** the note id, the note title, and the aggregated note badge specified in §1.AC.10 (when count > 0).
- **Subline:** the note type and the note's claim count (e.g., `*Requirement* · 65 claims`).
- **Body region:** a fixed-height vertically-scrollable container (`max-height: ~360px`, `overflow-y: auto`, `overflow-x: hidden`) holding the full rendered note body. The body MUST be sourced from `window.__scepterBodyMap[noteId]` populated by the body-map walk (§8.AC.01), NOT via eager `data-note-excerpt` emission (already forbidden by §7.AC.07). The body MUST be rendered through the SCEpter markdown-it plugin so claim references inside the body remain hoverable, badge-bearing on local claim definitions, and clickable for navigation.
- **Footer:** the note's file path and an "Open Note →" link. The footer MUST sit OUTSIDE the scrollable body region so it remains visible regardless of scroll position inside the body.
...
§7.AC.06 The note excerpt MUST be capped at a reasonable line count (default: 50 lines) with a `*…content continues*` truncation marker on rendering surfaces that CANNOT host a scrollable container — specifically the editor hover (VS Code `MarkdownString`) and any other flat-hover surface where overflowing the visible area degrades usability. The cap exists to prevent a long note from blowing out a flat hover; it does not exist to ration content where the surface can scroll. The markdown preview note hover MUST NOT apply this cap (see §3.AC.09): it places the body inside a fixed-height scrollable region, so the visual budget is bounded by the container, not by the line count, and the user benefits from having the entire note available without re-navigating. The resolver MUST expose an opt-in uncapped path (e.g., `resolveNoteBodySync(noteId, { uncapped: true })`) keyed separately in its cache from the capped path so both modes coexist.
...
§8.AC.06 Per-span data-attribute computation in `buildDataAttrs` MUST be O(1) amortized — measured by the work the plugin does for ONE rendered span, not the work it does to populate the indices that span queries. Aggregations whose natural shape is O(claims × crossRefs) — specifically the aggregated note hover badge (§1.AC.10) — MUST be cached at the index level (lazy, populated on first request, dropped on `refresh()` and `switchProject()`) so per-span lookups become O(1) cache reads. The plugin MUST additionally skip the badge computation when rendering a body excerpt (`env._scepterLineOffset` is defined) — the badge is not user-visible inside a nested excerpt and the work is wasted. **Anti-pattern:** computing the aggregated badge inline per span (e.g., calling `claimsForNote(noteId)` plus `incomingRefs(claim.fqid)` for every claim every time `buildDataAttrs` runs for a note span) recreates the architectural shape of the abolished `buildExcerptCache` pass and starves the host event loop on large projects. The 2026-05-03 incident is the binding precedent.

§8.AC.07 `ClaimBodyResolver.resolveTransitive` MUST honor a wall-clock budget (default: 250ms) that bounds the worst-case render latency of one body-map walk. When the budget is exceeded the BFS MUST abort and return whatever bodies it has accumulated; the preview tooltip MUST degrade gracefully (unreached refs surface "no nested body" rather than block the user). The wall-clock budget pairs with the depth and `maxBodies` caps as a third independent bound — necessary because depth and body-count caps alone do not bound render time when individual bodies are large or contain many cross-refs that the plugin lookups while rendering. The budget MUST be a constant (not a runtime knob) so the bound is part of the shipped contract.

§8.AC.08 Uncapped note body rendering inside `resolveTransitive` MUST be bounded by TWO additional sub-budgets independent of `maxBodies`: (a) a per-walk count limit on uncapped renders (default: 4 — additional reachable notes degrade to the capped excerpt), and (b) a hard line-count ceiling per uncapped body (default: 5000 lines — pathologically large notes route to the capped path regardless of the uncapped flag). The justification: an uncapped note body is roughly an order of magnitude more expensive to render than a claim body (longer content × more refs through markdown-it × per-span index lookups), and rendering several at once compounds the cost. The two sub-budgets together ensure that even a document seeded with 30+ note mentions cannot stall the host.
...
- 2026-05-05: Preview hover surface extension. §1 was extended with three new ACs (§1.AC.09 claim-hover badge mirror, §1.AC.10 aggregated note-hover badge, §1.AC.11 hover-only surface scoping) so the inline badge primitive surfaces inside both the claim and note hovers in the markdown preview. The preview note hover was promoted from a one-line excerpt into a full-body layout: §3.AC.09 specifies a header + badge + subline + fixed-height scrollable body region + persistent footer. §7.AC.06 was carved to allow uncapped note rendering on the preview surface only — the editor hover and other flat surfaces retain the 50-line cap. The body-map walk in §8.AC.01 is the carrier for the uncapped note body (via a new uncapped path on `resolveNoteBodySync` and a note-id branch in `resolveTransitive`). Implementation lands in `vscode/src/markdown-plugin.ts` (badge data-attr emission for claims and notes), `vscode/src/claim-body-resolver.ts` (uncapped note body, note-id seed handling), `vscode/media/preview-script.js` (claim-hover badge in header, rewritten note hover layout), and `vscode/media/markdown-claims.css` (`.scepter-note-body` scroll region rule). Total AC count rose from 65 to 69.
...
- 2026-07-06: {T013} fixed two extension-side defects in the §2.AC.10–§2.AC.13 section-hover surface: (1) `decoration-provider.ts`'s `sectionDecoration` carried no `color:` property, so a *resolved* section reference rendered visually indistinguishable from unresolved/plain text; (2) `claim-index.ts`'s `readSectionContent` clamped the body excerpt to 200 lines even when a real `endLine` was known, truncating §2.AC.10(e)'s body excerpt for sections longer than 200 lines. Both are now fixed. Cross-link added in graph-hygiene pass; no AC/DC change. **Open traceability-matrix gap:** §2 has no AC/DC pinning that a resolved section reference's *decoration* MUST match the resolved-style visual treatment (color/pointer/overview-ruler) that claim references get — the defect existed because no claim governed this surface. This is asymmetric with the cross-project case, where {R011.§4.AC.05} and {DD015.§1.DC.07} explicitly pin the decoration hue/style contract. No DD exists that documents the local (non-cross-project) hover/decoration implementation surface at the design-decision level — {DD012} and {DD013} are adjacent (library-API migration, sidebar views) but don't cover this. Flagged for a future author to close with an explicit AC if the gap recurs; not invented here per linker discipline.
