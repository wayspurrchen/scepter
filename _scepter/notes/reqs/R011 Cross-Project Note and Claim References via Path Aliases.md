...
- 2026-07-05: {T011} closed a conformance gap in the shipped implementation of §2.AC.06/§4.AC.07: `PeerProjectResolver.lookupClaim` was rebuilding the peer's claim index from scratch on every call rather than caching it per alias as {DD015} designed, freezing the VS Code extension host on large peer projects. Fixed via a new `peerIndexCache`/`ensurePeerIndexBuilt()` on `PeerProjectResolver`. No AC text change — see {DD015}'s Status log for the fix detail.
