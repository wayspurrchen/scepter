...
- {T011} (2026-07-05) — Conformance-gap fix: `PeerProjectResolver.lookupClaim` was rebuilding the peer's entire claim index from scratch on every call instead of querying an already-built index (line 150, line 230 of this DD); added `peerIndexCache`/`ensurePeerIndexBuilt()` to realize §2.AC.06/§4.AC.07 as designed.
...
- 2026-07-05: {T011} found and closed a conformance gap between this DD's design and the shipped implementation. Line 150 specifies `lookupClaim(aliasPrefix, claimAddress)` resolving alias then querying the peer's already-built `claimIndex.getClaim(fqid)`; the §4.AC.07 row at line 230 specifies the peer-index cache as owned by core's `PeerProjectResolver`. The actual `PeerProjectResolver.lookupClaim` instead called `claimIndex.build(peerNotesWithContent)` from scratch on every invocation — the pre-existing `peerCache` cached only the peer `ProjectManager`, not the built index. On a peer project of ~1,942 notes this froze the VS Code extension host for seconds per hover/goto-definition/refresh (multiplied by one rebuild per cross-project reference in `validateCrossProjectReferences`). Fixed by adding a `peerIndexCache: Map<string, Promise<void>>` + `ensurePeerIndexBuilt()` to `PeerProjectResolver`, cleared alongside the existing `peerCache` in `invalidate()`/`invalidateChanged()` so the §4.AC.12 invalidation invariant still holds. Realizes {R011.§4.AC.07} / {R011.§2.AC.06} as designed. No AC/DC numbering change; no design revision — the implementation now matches what was already specified here.
