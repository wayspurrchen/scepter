...
6. **Data-accuracy precondition (2026-07-06)**: This design's entire partial-coverage output is only as good as the underlying claim index's mention-parsing accuracy — {DD005.§DC.01}'s "at least one column empty" gap signal is meaningless if a column can be silently empty due to a parser bug rather than a genuine absence of coverage. {T012} documents and fixes exactly this failure mode: the `note-parser.ts` source-annotation regex dropped the claim path on section-less `§`-before-prefix citations, making live `@implements` blocks invisible to the Source column and producing false partial-coverage rows in this command's output. Not a change to this DD's own logic — a fix to a precondition it depends on. `partial-coverage-gaps` and `traceability` suites were re-run as part of T012's regression pass.
